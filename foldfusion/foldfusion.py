...
class FoldFusion:
    ...
    def _run_dogsite3(self, af_model_path: Path, output_dir: Path) -> Path:
        ...
        try:
            ...
            # Apply optional execution controls
            dogsite3.timeout = self.config.dogsite3_timeout_seconds
            dogsite3.memory_limit_mb = self.config.dogsite3_memory_mb
    ...
    def _run_siena(self, best_edf: Path, output_dir: Path) -> tuple[Siena, list]:
        ...
        try:
            siena = Siena(
                ...
                output_dir,
                self.config.siena_identity,
            )
            # Apply optional execution controls
            siena.timeout = self.config.siena_timeout_seconds
            siena.memory_limit_mb = self.config.siena_memory_mb
    ...
    def _run_ligand_extraction(
        ...
        try:
            ...
            # Apply optional execution controls
            ligand_extractor.timeout = self.config.ligand_extractor_timeout_seconds
            ligand_extractor.memory_limit_mb = self.config.ligand_extractor_memory_mb
    ...
    def _run_jamda_scorer(
        ...
        try:
            ...
            # Apply optional execution controls
            jamda_scorer.timeout = self.config.jamda_timeout_seconds
            jamda_scorer.memory_limit_mb = self.config.jamda_memory_mb
    ...
    def run(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # Apply optional execution controls
                siena_db.timeout = self.config.siena_db_timeout_seconds
                siena_db.memory_limit_mb = self.config.siena_db_memory_mb
            ...
            def _task(uid: str) -> str:
                ...
                failed_marker = output_dir / "_FAILED"
                if output_dir.exists():
                    if self.config.resume and success_marker.exists():
                        logger.info("Skipping %s: already completed (_SUCCESS)", uid)
                        return uid
                    if self.config.skip_failed_ids and failed_marker.exists():
                        logger.warning("Skipping %s: marked as failed (_FAILED)", uid)
                        return uid
            ...
            if max_workers == 1:
                ...
                for i, uid in enumerate(self.config.uniprot_ids, 1):
                    ...
                    try:
                        ...
                        # Mark failed
                        try:
                            out_dir = main_output_dir / "Results" / uid
                            out_dir.mkdir(parents=True, exist_ok=True)
                            (out_dir / "_FAILED").write_text(f"error: {error_reason}\n")
                        except Exception:
                            pass
...
def main() -> None:
    """CLI entry point for the FoldFusion pipeline.

    Usage:
        foldfusion [config_path]

    Defaults to `config.toml` in the current working directory if no path is
    provided. This function is wired as the console script in pyproject.toml
    and also used by `python -m foldfusion`.
    """
    import argparse
    from pathlib import Path as _Path

    parser = argparse.ArgumentParser(
        prog="foldfusion", description="FoldFusion pipeline runner"
    )
    parser.add_argument(
        "config",
        nargs="?",
    ...
    pipeline = FoldFusion(str(cfg_path))
    pipeline.run()
