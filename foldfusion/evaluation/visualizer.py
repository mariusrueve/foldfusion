...
import plotly.offline as plotly_offline  # type: ignore[import-untyped]
...
def visualize_protein_with_radius(
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            hovertemplate=(
                "Experimental<br>X: %{x}<br>Y: %{y}<br>Z: %{z}<extra></extra>"
            ),
        )
    )
...
def visualize_local_rmsd_region(
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            hovertemplate=(
                "AlphaFold (excluded)<br>X: %{x}<br>Y: %{y}<br>Z: %{z}<extra></extra>"
            ),
        )
    )
    ...
    if np.any(af_within_radius):
        fig.add_trace(
            go.Scatter3d(
                ...
                hovertemplate=(
                    "AlphaFold (RMSD)<br>X: %{x}<br>Y: %{y}<br>Z: %{z}<extra></extra>"
                ),
            )
        )
