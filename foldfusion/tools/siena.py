...
class Siena(Tool):
    ...
    def __init__(
        ...
        """Store inputs and construct the Siena command.

        Args:
            executable: Path to the Siena alignment executable.
            edf: EDF file describing the DogSite3 pocket to align against.
            siena_db: Location of the pre-built Siena database.
            pdb_directory: Directory holding the experimental PDB structures
                referenced by the database.
            output_dir: Pipeline output directory; results are stored under its
                `Siena/` subdirectory.
            identity: Optional active-site identity cutoff (0.0-1.0).
        ...
        # Active-site identity threshold for SIENA filtering (0.0 - 1.0)
        # Defaults to 0.85 if not provided
        self.identity_threshold = 0.85 if identity is None else float(identity)
        ...
        try:
            ...
            for _, row in top_results.iterrows():
                ...
                active_site_identity = (
                    float(row["Active site identity"])
                    if "Active site identity" in row
                    and pd.notna(row["Active site identity"])
                    else None
                )
