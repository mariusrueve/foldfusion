...
class Dogsite3(Tool):
    ...
    def __init__(self, executable: Path, pdb_file: Path, output_dir: Path):
        """Prepare the DoGSite3 wrapper and validate its inputs.

        Args:
            executable: Path to the DoGSite3 CLI binary.
            pdb_file: PDB structure that should be analysed.
            output_dir: Pipeline output directory; a `Dogsite3/` subdirectory is
                created inside it.
    ...
    def get_command(self) -> list[str]:
        """Assemble the DoGSite3 command line with validated arguments."""
