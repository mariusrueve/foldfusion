...
class Config:
    ...
    def __init__(self, config_path: Path) -> None:
        ...
        # Early validation for mutually exclusive/required options that
        # tests expect to be enforced at construction time.
        ids_list = self.dict.get("uniprot_ids")
        ids_file = self.dict.get("uniprot_ids_file")
        if ids_list is not None and ids_file is not None:
            raise ValueError(
    ...
    @property
    def siena_identity(self) -> float:
        """Active-site identity threshold for SIENA alignments.

        Returns:
            Float in the range [0.0, 1.0] representing the minimum active-site
            identity required for SIENA alignments to be considered.

        Raises:
            TypeError: If the configured value is not a number.
            ValueError: If the value is outside the [0.0, 1.0] range.
        """
        value = self.dict.get("siena_identity", 0.85)
        if not isinstance(value, int | float):
            error_msg = "siena_identity must be a number (float between 0 and 1)"
            logger.error(error_msg)
            raise TypeError(error_msg)

        v = float(value)
        if not (0.0 <= v <= 1.0):
            error_msg = "siena_identity must be between 0.0 and 1.0"
            logger.error(error_msg)
            raise ValueError(error_msg)

        logger.debug(f"Using SIENA identity threshold: {v}")
        return v
    ...
    def siena_db_database_path(self) -> Path | None:
        ...
        if isinstance(value, Path):
            ...
            raise ValueError(
                "siena_db_database_path must be a string or Path if provided"
            )
    ...
    # --- Robustness & Limits -------------------------------------------------
    # Optional: timeouts (seconds) for each external tool; None = no timeout
    @property
    def siena_timeout_seconds(self) -> int | None:
        val = self.dict.get("siena_timeout_seconds")
        return int(val) if isinstance(val, int) else None

    @property
    def siena_db_timeout_seconds(self) -> int | None:
        val = self.dict.get("siena_db_timeout_seconds")
        return int(val) if isinstance(val, int) else None

    @property
    def dogsite3_timeout_seconds(self) -> int | None:
        val = self.dict.get("dogsite3_timeout_seconds")
        return int(val) if isinstance(val, int) else None

    @property
    def ligand_extractor_timeout_seconds(self) -> int | None:
        val = self.dict.get("ligand_extractor_timeout_seconds")
        return int(val) if isinstance(val, int) else None

    @property
    def jamda_timeout_seconds(self) -> int | None:
        val = self.dict.get("jamda_timeout_seconds")
        return int(val) if isinstance(val, int) else None

    # Optional: memory ceilings (MB) for each external tool; None = no limit
    @property
    def siena_memory_mb(self) -> int | None:
        val = self.dict.get("siena_memory_mb")
        return int(val) if isinstance(val, int) else None

    @property
    def siena_db_memory_mb(self) -> int | None:
        val = self.dict.get("siena_db_memory_mb")
        return int(val) if isinstance(val, int) else None

    @property
    def dogsite3_memory_mb(self) -> int | None:
        val = self.dict.get("dogsite3_memory_mb")
        return int(val) if isinstance(val, int) else None

    @property
    def ligand_extractor_memory_mb(self) -> int | None:
        val = self.dict.get("ligand_extractor_memory_mb")
        return int(val) if isinstance(val, int) else None

    @property
    def jamda_memory_mb(self) -> int | None:
        val = self.dict.get("jamda_memory_mb")
        return int(val) if isinstance(val, int) else None

    @property
    def resume(self) -> bool:
        """Skip UniProt IDs that are already marked as completed (_SUCCESS)."""
        return bool(self.dict.get("resume", True))

    @property
    def skip_failed_ids(self) -> bool:
        """Skip IDs that are marked as failed (_FAILED) on reruns."""
        return bool(self.dict.get("skip_failed_ids", True))
