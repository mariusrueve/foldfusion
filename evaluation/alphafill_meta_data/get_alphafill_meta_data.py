...
def http_get_json(url: str, timeout: float = 30.0, headers: t.Optional[dict] = None) -> t.Any:
	req = Request(url, headers={"User-Agent": "foldfusion/alphafill-meta-fetcher" , **(headers or {})})
...
def find_candidate_aff_ids_from_3dbeacon(uniprot_id: str, timeout: float = 30.0) -> list[str]:
...
def fetch_alphafill_metadata(uniprot_id: str, *, timeout: float = 30.0, sleep_between: float = 0.2) -> FetchResult:
	...
	try:
		...
		if e.code != 404:
			return FetchResult(uniprot_id=uniprot_id, alphafill_id=None, status="error", error=f"HTTP {e.code}: {e.reason}")
		# 404 -> try to discover via 3d-beacon
	except URLError as e:
		return FetchResult(uniprot_id=uniprot_id, alphafill_id=None, status="error", error=f"URL error: {e.reason}")
	except Exception as e:
		return FetchResult(uniprot_id=uniprot_id, alphafill_id=None, status="error", error=str(e))
...
def main(argv: list[str]) -> int:
	...
	for i, uid in enumerate(uniprot_ids, 1):
		result = fetch_alphafill_metadata(uid, timeout=45.0, sleep_between=sleep_between)
		if result.status == "ok" and result.alphafill_id:
			try:
				...
				errors += 1
				print(f"[{i}/{len(uniprot_ids)}] {uid}: error saving {result.alphafill_id}: {e}")
